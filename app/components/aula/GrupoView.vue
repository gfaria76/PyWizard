<template>
  <div class="space-y-8">
    ...
    >
      <header class="mb-5">
        <span class="inline-block font-label-caps text-[10px] tracking-widest text-primary-fixed uppercase">
          Expedição da Guilda · 5 integrantes
        </span>
        <h2 class="font-headline-sm text-xl text-primary-fixed mt-1 uppercase tracking-wider">
          {{ block.title }}
        </h2>
      </header>
    ...
    <section v-if="closingSections.length" id="sec-fechamento" class="glass-panel rounded-xl p-6 md:p-8">
      ...
      <div class="space-y-5">
        <div
          ...
          :key="section.key"
          class="bg-surface-container-low/40 rounded-lg p-4 md:p-5 border-l-2 border-primary-fixed/40"
        >
          <h3 class="font-label-caps text-[11px] tracking-widest text-primary-fixed mb-2 uppercase">
            {{ section.title }}
          </h3>
