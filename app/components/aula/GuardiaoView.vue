      ...
      <header class="mb-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
          <span class="inline-block font-label-caps text-[10px] tracking-widest text-tertiary-fixed-dim uppercase">
            Trilha Guardião · Revisão acumulativa
          </span>
        ...
        <span
          v-if="block.missions.length"
          class="font-code-md text-xs text-on-surface-variant opacity-70"
        >
          {{ block.missions.length }} desafios
        </span>
      </header>
